...
//      cv::Mat img = imread(argv[1], cv::IMREAD_GRAYSCALE);
    ...
    std::pair<std::vector<int>, int> new_img_seq = MarkComponents(&array_seq, img.rows, img.cols);
    ...
    std::pair<std::vector<int>, int> new_img_seq_0 = MarkComponentsSeq(&array_seq_0, img.rows, img.cols);
    end = std::chrono::high_resolution_clock::now();
    diff = end - start;
    std::cout << "Normal Sequential: " << std::setw(9) << diff.count() << "\n";
    std::vector<cv::Vec3b> result_image_seq_0(new_img_seq_0.first.begin(), new_img_seq_0.first.end());
    ...
    start = std::chrono::high_resolution_clock::now();
    std::pair<std::vector<int>, int> new_img_tbb = MarkComponentsParTbb(&array_tbb, img.rows, img.cols, 4);
    end = std::chrono::high_resolution_clock::now();
    diff = end - start;
